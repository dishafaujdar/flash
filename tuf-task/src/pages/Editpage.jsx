...
const ManageCards = () => {
  ...
  useEffect(() => {
    axios.get(`${process.env.REACT_APP_API_URL || 'http://localhost:3000'}/flashcards`)
  ...
  const handleCreate = () => {
    if (newFlashcard.question && newFlashcard.answer) {
      axios.post(`${process.env.REACT_APP_API_URL || 'http://localhost:3000'}/flashcards`, newFlashcard)
        .then(response => {
  ...
  const handleUpdate = () => {
    if (newFlashcard.question && newFlashcard.answer) {
      axios.put(`${process.env.REACT_APP_API_URL || 'http://localhost:3000'}/flashcards/${currentIndex}`, newFlashcard)
        .then(() => {
          setCards(cards.map(fc => (fc.id === currentIndex ? { ...fc, ...newFlashcard } : fc)));
          handleCloseDialog();
        })
  ...
  const handleDelete = (id) => {
    axios.delete(`${process.env.REACT_APP_API_URL || 'http://localhost:3000'}/flashcards/${id}`)
      .then(() => {
  ...
  return (
    <Box display="flex" flexDirection="column" alignItems="center" p={2} className='page'>
      ...
      <div className='cards-contanier'>
        <Box mt={2} display="flex" gap="16px" padding="16px" flexWrap="wrap">
          {cards.map(fc => (
